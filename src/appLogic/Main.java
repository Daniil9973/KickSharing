            ...
            switch (choice) {
                ...
                case "6": // Все поездки
                    ...
                    if (rides.isEmpty()) {
                        ...
                        System.out.println("\n===СПИСОК ВСЕХ ПОЕЗДОК===");
                        for (Ride r : rides) {
                            System.out.println(r.getId() + " | user_id: " + r.getUserId() + " | " + r.getStartTime() + " - " + r.getEndTime() + " | " + r.getPrice());
                        }
                    }
                    break;

                case "7": // Найти поездку по ID
                    try {
                        System.out.print("ID поездки: ");
                        int rideId = Integer.parseInt(scanner.nextLine());
                        Ride ride = logic.getRideById(rideId);
                        if (ride != null) {
                            System.out.println("Найдена: ID=" + ride.getId() + " | user_id=" + ride.getUserId() + " | " + ride.getStartTime() + " - " + ride.getEndTime() + " | " + ride.getPrice());
                        } else {
                ...
                case "8": // Все платежи
                    ...
                    if (payments.isEmpty()) {
                        ...
                        for (Payment p : payments) {
                            System.out.println(p.getId() + " | ride_id: " + p.getRideID() + " | сумма: " + p.getAmount() + " | статус: " + p.getStatus());
                        }
                    }
                    break;

                case "9": // Найти платёж по ID
                    try {
                        System.out.print("ID платежа: ");
                        int paymentId = Integer.parseInt(scanner.nextLine());
                        Payment payment = logic.getPaymentById(paymentId);
                        if (payment != null) {
                            System.out.println("Найден: ID=" + payment.getId() + " | ride_id=" + payment.getRideID() + " | сумма=" + payment.getAmount() + " | статус=" + payment.getStatus());
                        } else {
